...
class Seq2SeqModel(object):
  ...
  def __init__(self, source_vocab_size, target_vocab_size, buckets, size,
               ...
               num_samples=512, forward_only=False,temp=1.0):
    """Create the model.
    ...
    self.loss_summaries = []
    if forward_only:
      self.outputs, self.losses,self.wids = seq2seq.model_with_buckets(
          self.encoder_inputs, self.decoder_inputs, targets,
          self.target_weights, buckets, self.target_vocab_size,
          lambda x, y: seq2seq_f(x, y, True,True),
          softmax_loss_function=softmax_loss_function)
      for i in xrange(len(self.losses)):
          self.loss_summaries.append(tf.scalar_summary("loss{}".format(i), self.losses[i]))
    ...
    if not forward_only:
      ...
      print('use aggregation_method=2')
      for b in xrange(len(buckets)):
  ...
  def step(self, session, encoder_inputs, decoder_inputs, target_weights,
    ...
    if not forward_only:
      output_feed = [self.loss_summaries[bucket_id],self.updates[bucket_id],  # Update Op that does SGD.
                     self.gradient_norms[bucket_id],  # Gradient norm.
                     self.losses[bucket_id]]  # Loss for this batch.
    else:
      output_feed = [self.loss_summaries[bucket_id],self.losses[bucket_id]]  # Loss for this batch.
      for l in xrange(len(self.wids[bucket_id])):
        output_feed.append(self.wids[bucket_id][l])

    outputs = session.run(output_feed, input_feed)
    m = outputs[0]
    if not forward_only:
        self.trainwriter.add_summary(m,self.global_step.eval())
    else:
        self.devwriter.add_summary(m,self.global_step.eval())
    if not forward_only:
