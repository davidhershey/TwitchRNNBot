...
def _get_predicted_sentence(input_sentence, vocab, rev_vocab, model, sess):
    ...
    _, _, output_wids = model.step(sess, encoder_inputs, decoder_inputs, target_weights, bucket_id, forward_only=True)
    ...
    for wid in output_wids:
        # top_words = logit[0].argsort()[-3:][::-1]
        # select_word_number = top_words[0]
        # if select_word_number==3:
        #     select_word_number= top_words[1:]
        # select_word_number = int(decoding_enhanced.sample_with_temperature(logit[0], TEMPERATURE))
        select_word_number = wid
